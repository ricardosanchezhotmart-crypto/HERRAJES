    ...
    <header className="sticky top-0 z-40 border-b border-border/60 bg-background/80 backdrop-blur-xl">
      <div className="container flex h-16 items-center justify-between gap-4">
        ...
        <button
          ...
          className="hidden h-10 flex-1 max-w-md items-center gap-2 rounded-full border border-border bg-muted/40 px-4 text-sm text-muted-foreground transition hover:border-foreground/20 hover:bg-muted sm:flex"
          aria-label="Buscar"
        >
        ...
        <div className="flex items-center gap-1">
          <button
            onClick={() => setSearchOpen(true)}
            className="flex h-10 w-10 items-center justify-center rounded-full transition hover:bg-muted sm:hidden"
            aria-label="Buscar"
          >
          ...
          <Link
            href="/#categorias"
            className="hidden rounded-full px-3 py-2 text-sm transition hover:bg-muted md:inline-block"
          >
            Categorías
          </Link>
